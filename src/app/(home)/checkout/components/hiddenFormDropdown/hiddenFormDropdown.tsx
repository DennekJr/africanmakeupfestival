...
import { CssSelectField } from "../../../checkout/components/utils";
...
export const HiddenFormDropdown = ({
  ...
  const [tickets, setTickets] = React.useState<
    { ticketName: string; value: number }[]
  >([]);
  const {
    tickets: Tickets,
    myTicket,
  ...
  const handleChange = (e) => {
    ...
    dispatch(
      setBillingInfo({
        ...
        data: Object.values(billingInfo)
      })
    );
  ...
  return (
    <Box className="space-y-6 px-0">
      ...
      <div className="space-y-2 lg:col-span-6" style={{ position: "relative" }}>
        {title.includes("Assign") && myTicket.value == 0 && (
          <label
            className="peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-base leading-[22.4px] font-normal text-[#1E1C21]">
            Select your ticket
          </label>
            ...
            <Box className={"lg:col-span-6 space-y-16"}>
              {currentLeftOverTickets.flatMap((ticket) => {
                return Array.from(
                  ...
                  (_, ticketIndex) => {
                    return (
                      <Box key={`${ticket.ticketName}-${ticketIndex}`}>
                        ...
                        <BillingForm
                          ticket={`${ticket.ticketName}-${ticketIndex}`}
                        />
                      </Box>
                    );
                  }
