...
export const BoothsSlider = () => {
  ...
  const [addedAddOns, setAddedAddOns] = useState(addOns);
        ...
        {SliderBoothAddOn.map((boothAddOn, index) => {
          ...
          >
            <div
              ...
              role="button"
              className={"grid gap-4 border border-input transition-colors ease-linear duration-200 rounded-[calc(1rem+6px)] p-2 " + (addOns.includes(boothAddOn.name) && 'border-[#504E56]')}
            >
              ...
              <div className="grid grid-cols-2 text-xl text-[#2C2A32] px-2">
                ...
                <p className="text-right">NGN {formatCurrency(Number(boothAddOn.price))}</p>
              </div>
            </div>
          </div>
        })}
