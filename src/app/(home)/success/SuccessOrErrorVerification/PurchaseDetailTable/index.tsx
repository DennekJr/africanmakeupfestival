...
export const PurchaseDetailTable = ({ metaData, total, currency }: {
  metaData: {
    buyerForm: { [ticket: string]: { name: string; value: string }[] }[] | ExhibitionBoothBillingInfo,
  ...
  const mappedDataArray = metaData.otherTicketForms === undefined ? [] : (otherTicketForms as TicketBilingInfo[]).map(ticket => {
  ...
  return (
    <Box className={"w-full"}>
      ...
      {Object.keys(metaData.buyerForm).includes("form_booth") &&
        <Box>
          <Box className={"flex text-lightSecondary justify-between p-4 border-b border-midGrey"}>
            <Box>Booth</Box>
      ...
      {!Object.keys(metaData.buyerForm).includes("form_booth") && mainBuyer[0].map((ticketData, index) => {
